...
export default function SmsPage() {
  ...
  const handleScheduleSMS = () => {
    ...
    if (scheduleTime) {
      alert(`SMS scheduled for ${scheduleTime}`)
    }
  }
  ...
  const handleResendSMS = async (sms: any) => {
    if (confirm(`Resend SMS to ${sms.to_number}?`)) {
      try {
        const response = await api.post('/sms', {
          action: 'resend',
          data: {
        ...
        toast({
          title: "SMS Resent",
          description: `SMS resent to ${sms.to_number}`,
        })
                ...
                {smsTemplates.map((template) => (
                    ...
                    >
                      ...
                      <div className="text-right">
                          <p className="text-sm text-gray-600 font-medium">Used {template.usage_count} times</p>
                      </div>
                    </div>
