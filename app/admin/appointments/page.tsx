...
export default function AdminAppointmentsPage() {
  ...
  const handleBookAppointment = async () => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        toast({
          title: "Error",
          description: `Error booking appointment: ${result.error}`,
          variant: "destructive",
        })
      }
    } catch (e) {
  ...
  const handleRescheduleAppointment = (appointment: any) => {
    ...
    toast({
      ...
      description: `Rescheduling appointment for ${appointment.patient_id}`,
    })
  ...
  const debugFetchAll = async () => {
    ...
    try {
      const response = await api.get('/appointments', {
        params: { page: 1, pageSize: 5, viewAll: true }
      })
      
      console.log('Total appointments:', response.data.totalCount || 0)
      console.log('Debug result:', response.data)
      if (response.data.appointments && response.data.appointments.length > 0) {
        console.log('First appointment:', response.data.appointments[0])
        console.log('Patient data:', response.data.appointments[0].patient)
        console.log('Dentist name:', response.data.appointments[0].dentist_name)
      }
    } catch (error) {
                              ...
                              <Avatar className="h-8 w-8 border-2 border-gray-100">
                                ...
                                <AvatarFallback className="bg-gradient-to-r from-blue-500 to-purple-600 text-white text-xs font-bold">
                                  {appointment.patient?.first_name?.trim() && appointment.patient?.last_name?.trim() ? 
                                    `${appointment.patient.first_name.trim()[0]}${appointment.patient.last_name.trim()[0]}` : "N/A"}
                                </AvatarFallback>
                      </Avatar>
                              <div>
