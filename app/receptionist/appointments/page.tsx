...
export default function ReceptionistAppointmentsPage() {
  ...
  // Handle pagination updates when total count changes
  useEffect(() => {
    const newTotalPages = Math.ceil(totalAppointments / pageSize)
  ...
  }, [totalAppointments, pageSize, currentPage])
  ...
  const handleBookAppointment = async () => {
    ...
    // Validate that the selected patient has proper data
    const selectedPatient = patients.find(p => p.id === bookingForm.patient_id)
    if (!selectedPatient) {
      toast({
        title: "Error",
    ...
    // Check if patient has proper name data
    if (!selectedPatient.first_name?.trim() || !selectedPatient.last_name?.trim()) {
      toast({
        title: "Warning",
        description: "Selected patient has incomplete information. Please update patient details first.",
        variant: "destructive",
      })
      return
    }
    ...
    try {
      ...
      if (response.data.success) {
        // Add new appointment to local state immediately
        setAppointments(prevAppointments => [response.data.appointment, ...prevAppointments])
  ...
  const debugFetchAll = async () => {
    console.log('Debug: Fetching all appointments without filters...')
    ...
    try {
      ...
      console.log('Total appointments in database:', response.data.totalCount || 0)
      
      if (response.data.success && response.data.appointments && response.data.appointments.length > 0) {
      ...
      if (data && data.length > 0) {
        ...
        console.log('Patient data:', data[0].patient)
        console.log('Dentist name:', data[0].dentist_name)
      }
    } else {
  ...
  const filteredAppointments = appointments.filter(appointment => {
    // Debug: Log each appointment being filtered
    console.log('Filtering appointment:', {
      id: appointment.id,
      patientName: appointment.patient?.first_name && appointment.patient?.last_name ? 
        `${appointment.patient.first_name} ${appointment.patient.last_name}` : 'Unknown',
      status: appointment.status,
      treatment: appointment.treatment,
      searchTerm,
      filterStatus,
    ...
    const result = matchesSearch && matchesStatus && matchesTreatment
    console.log('Appointment filter result:', result)
        ...
        <div className="p-4 space-y-4">
          ...
          <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4">
            <div>
              ...
              <p className="text-gray-600 text-sm lg:text-base">Schedule and manage patient appointments efficiently</p>
                            ...
                            {patients.map((p) => (
                              <SelectItem key={p.id} value={p.id}>
                                {p.first_name?.trim() && p.last_name?.trim() ? 
                                  `${p.first_name.trim()} ${p.last_name.trim()}` : 
                                  p.phone ? `Patient (${p.phone})` : 'Unknown Patient'}
                              </SelectItem>
                            ))}
                          </SelectContent>
                        </Select>
                              ...
                              <div>
                                <div className="font-semibold text-gray-900">
                                  {appointment.patient?.first_name?.trim() && appointment.patient?.last_name?.trim() ? 
                                    `${appointment.patient.first_name.trim()} ${appointment.patient.last_name.trim()}` : 
                                    appointment.patient?.phone ? `Patient (${appointment.patient.phone})` : 'Unknown Patient'}
                                </div>
                        </div>
                      </div>
                          </td>
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <p className="text-lg">{selectedAppointment.dentist_name || 'Not assigned'}</p>
                </div>
                <div>
